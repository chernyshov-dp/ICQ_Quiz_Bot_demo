...
question1 = ['Шахматный скакун', 'Свадебный клич', 'Самая важная для человека жидкость', '"Ребёнок" среднего рода',
                                                                                         'Какую геометрическую фигуру '
                                                                                         'украшают бриллиантами']
question3 = ['Чему равен факториал нуля', 'Солнце это', '"В подворотне нас ждёт ...,  хочет нас посадить на крючок" ('
                                                        'песня)', 'Настоящий учёный остаётся учёным и во сне: '
                                                                  'создатель периодической системы химических '
                                                                  'элементов (фамилия)', 'ОНИ есть только у слонов']

answers = {'q1': ['конь', 'горько', 'вода', 'чадо', 'кольцо'],
           'q2': ['японского', 'чёрный', 'черный', 'африка', 'чёрная дыра', 'черная '
                                                                            'дыра', 'природа'],
           'q3': ['1', 'звезда', 'маньяк', 'менделеев', 'слонята']}
...
def play_cb(bot, event):
    """ Вызов команды /play """
    # Id чата, из которого приходит сообщение
    user = event.data['chat']['chatId']
        ...
        if chats[f'{user}'][0] == 'q1':
            if event.data["text"][1:].lower() in answers['q1']:
                ...
                if question2 == 1:
                    ...
                    file_url = 'pictures/q2p4.jpg'
                elif question2 == 5:
                    file_url = 'pictures/q2p5.jpg'
